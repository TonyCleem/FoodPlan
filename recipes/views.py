...
def _apply_filters(recipes, filters):
    ...
    if filters.get("no_sugar"):
        recipes = recipes.filter(no_sugar=True)
    if filters.get("high_protein"):
        recipes = recipes.filter(high_protein=True)
    ...
    if filters.get("max_cost"):
        try:
            ...
            recipes = recipes.filter(
                id__in=[r.id for r in recipes if r.total_cost() <= max_cost]
            )
...
def dislike_recipe(request, recipe_id):
    ...
    try:
        ...
        if next_recipe:
            return redirect("recipes:recipe_details_with_id", recipe_id=next_recipe.id)
        return redirect("recipes:recipe_details")
    except Exception as e:
...
def user_login(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "auth.html",
            {"error": "Неверный email или пароль", "next": request.GET.get("next", "")},
        )
...
def register(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            return render(
                request, "registration.html", {"error": "Email уже зарегистрирован"}
            )
...
def lk(request):
    ...
    return render(
        request,
        "lk.html",
        {"liked_recipes": liked_recipes, "user": request.user, "profile": profile},
    )
